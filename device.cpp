...
static const char* REBOOT_MENU_NAMES[] = {
    "Reboot system",
    "Reboot recovery",
#ifdef DOWNLOAD_MODE
...
static const menu_entry REBOOT_MENU_ENTRIES[] = {
...
static const menu_entry MAIN_MENU_ENTRIES[] = {
    { ACTION_SUBMENU, { .submenu = &REBOOT_MENU } },
...
const char* const* Device::GetMenuItems() {
    size_t len = menu_stack.size();
    const menu* m = menu_stack.at(len - 1);
    return m->names;
}
...
Device::BuiltinAction Device::InvokeMenuItem(int menu_position) {
  ...
  if (entry->action_type == ACTION_SUBMENU) {
      menu_stack.push_back(entry->action.submenu);
      return NO_ACTION;
  }
