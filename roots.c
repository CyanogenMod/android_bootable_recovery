...
char* get_secondary_storage_path() {
    Volume* v;
    v = volume_for_path("/storage/sdcard1");
    if (v != NULL) {
    ...
    if (volume_for_path("/emmc"))
        return "/emmc";
    if (volume_for_path("/external_sd"))
        return "/external_sd";
    return NULL;
}
...
void setup_legacy_storage_paths() {
    ...
    char* secondary_path = get_secondary_storage_path();
    ...
    if (secondary_path != NULL) {
        if (strcmp("/external_sd", secondary_path) == 0) {
            mkdir(secondary_path, 0000);
        } else {
            symlink(secondary_path, "/external_sd");
        }
    }
}
