...
int nandroid_backup_md5_check(const char *backup_path) {
    DIR *dp;
    FILE *fd;
    int i = 0;
    int j = 0;
    ...
    if (dp != NULL) {
        ...
        while ((ep = readdir(dp)) && i < MAX_FILES_CHECKED) {
            if (strcmp(ep->d_name, ".") != 0 && strcmp(ep->d_name, "..") != 0
                ...
                snprintf(filepaths[i], PATH_MAX, "%s/%s", path, ep->d_name);
                i++;
            }
        }
        closedir(dp);
...
int nandroid_restore_md5_check(const char *backup_path, unsigned char flags) {
    ...
    int restore_num = check_boot + check_system + check_data + check_cache
                      + check_sdext + check_wimax;
    ...
    if (fd != NULL) {
        ...
        while (fgets(tmp, PATH_MAX, fd)) {
            ...
            strcpy(md5files[i], &tmp[HASH_LENGTH+2]);
            i++;
        }
        fclose(fd);
        md5count = i;
    ...
    for (i = 0; i < md5count; i++) {
        ...
        for (j = 0; j < filecount; j++) {
            if (strcmp(md5files[i], filenames[j]) == 0) {
                mm[i] = (MissingFiles){ 0, "", 0 };
                foundfile = 1;
            }
        }
        if (!foundfile) {
            mm[i] = (MissingFiles){ 1, md5files[i], strlen(md5files[i]) };
            totalmissing++;
        }
    }

    // Remove missing-mark from files that are not being restored
    for (i = 0; i < md5count; i++) {
        if ((!check_boot && strstr(mm[i].filename, "boot")) ||
                (!check_system && strstr(mm[i].filename, "system")) ||
                (!check_data && strstr(mm[i].filename, "data")) ||
                (!check_data && strstr(mm[i].filename, "android_secure")) ||
                (!check_cache && strstr(mm[i].filename, "cache")) ||
                (!check_sdext && strstr(mm[i].filename, "sd-ext")) ||
                (!check_wimax && strstr(mm[i].filename, "wimax"))) {
            mm[i] = (MissingFiles){ 0, "", 0 };
            totalmissing--;
        }
    }
    ...
    for (i = 0; i < filecount; i++) {
        ...
        for (j = 0; j < md5count; j++) {
            if (strcmp(filenames[i], md5files[j]) == 0) {
                mf[i] = (MissingFiles){ 0, "", 0 };
                foundfile = 1;
            }
        }
        if (!foundfile) {
            mf[i] = (MissingFiles){ 1, filenames[i], strlen(filenames[i]) };
            totalmissing++;
        }
    }
    ...
    // Remove missing-mark from files that are not being restored
    for (i = 0; i < filecount; i++) {
        if ((!check_boot && strstr(mf[i].filename, "boot")) ||
                (!check_system && strstr(mf[i].filename, "system")) ||
                (!check_data && strstr(mf[i].filename, "data")) ||
                (!check_data && strstr(mf[i].filename, "android_secure")) ||
                (!check_cache && strstr(mf[i].filename, "cache")) ||
                (!check_sdext && strstr(mf[i].filename, "sd-ext")) ||
                (!check_wimax && strstr(mf[i].filename, "wimax"))) {
            mf[i] = (MissingFiles){ 0, "", 0 };
            totalmissing--;
        }
    }
    ...
    for (i = 0; i < filecount; i++) {
        for (j = 0; j < md5count; j++) {
            if (strcmp(filenames[i], md5files[j]) == 0 && !mf[i].is_missing) { // need either mf or mm missing?
                if ((!check_boot && strstr(filenames[i], "boot")) ||
                        (!check_system && strstr(filenames[i], "system")) ||
                        (!check_data && strstr(filenames[i], "data")) ||
                        (!check_data && strstr(filenames[i], "android_secure")) ||
                        (!check_cache && strstr(filenames[i], "cache")) ||
                        (!check_sdext && strstr(filenames[i], "sd-ext")) ||
                        (!check_wimax && strstr(filenames[i], "wimax"))) {
                    continue;
                }
                if (calculate_md5(md5calc, filepaths[i]) != 0) {
                    ret = -1;
    ...
    ui_print("All MD5 sums verified\n");
