...
#define NANDROID_BACKUP_PATH /clockworkmod/backup/%F.%H.%M.%S

#define EMMC_PATH_SLASH (BOARD_EMMC_PATH STR(/))
#define EMMC_BACKUP_PATH (BOARD_EMMC_PATH STR(NANDROID_BACKUP_PATH))

#define EXTSDCARD_PATH_SLASH (BOARD_EXTSDCARD_PATH STR(/))
#define EXTSDCARD_BACKUP_PATH (BOARD_EXTSDCARD_PATH STR(NANDROID_BACKUP_PATH))
        ...
        case ITEM_BACKUP_TO_ADDITIONAL:
            {
                if (additional_storage != NULL)
                {
                    ...
                    {
                        if (!strcmp(additional_storage, BOARD_EMMC_PATH))
                            strftime(backup_path, sizeof(backup_path), EMMC_BACKUP_PATH, tmp);
                        else if (!strcmp(additional_storage, BOARD_EXTSDCARD_PATH))
                            strftime(backup_path, sizeof(backup_path), EXTSDCARD_BACKUP_PATH, tmp);
                    }
                    nandroid_backup(backup_path);
                }
            }
...
void create_fstab()
{
    ...
    write_fstab_root(BOARD_EMMC_PATH, file);
    write_fstab_root(BOARD_EXTSDCARD_PATH, file);
