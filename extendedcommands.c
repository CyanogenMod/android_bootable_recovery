...
void write_last_install_path(const char* install_path) {
    char path[PATH_MAX];
...
const char* read_last_install_path() {
    ...
    ensure_path_mounted(dirname(path));
    if (0 == stat(path, &info)) {
        char last_install_path[PATH_MAX];
        FILE *f = fopen(path, "r");
        fgets(last_install_path, PATH_MAX, f);
        fclose(f);

        return last_install_path;
    } else {
        return NULL;
    }
}
