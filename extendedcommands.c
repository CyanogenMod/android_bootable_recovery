...
int confirm_selection(const char* title, const char* confirm)
{
    ...
    if (one_confirm) {
        char* items[] = { "No",
                        strdup(confirm), //" Yes -- wipe partition",   // [1]
                        NULL };
        ...
        char* items[] = { "No",
                        ...
                        "No",
                        "No",
                        strdup(confirm), //" Yes -- wipe partition",   // [7]
                        "No",
                        "No",
                        "No",
                        NULL };
...
int show_partition_menu()
{
    ...
    char** options = malloc((num_volumes * 2) + 2);
    ...
    for (i = 0; i < num_volumes; ++i) {
        ...
        if(strcmp("ramdisk", v->fs_type) != 0 && strcmp("mtd", v->fs_type) != 0 && strcmp("emmc", v->fs_type) != 0 && strcmp("bml", v->fs_type) != 0) {
            if (strcmp("datamedia", v->fs_type) != 0) {
                sprintf(&mount_menu[mountable_volumes].mount[0], "mount %s", v->mount_point);
                sprintf(&mount_menu[mountable_volumes].unmount[0], "unmount %s", v->mount_point);
                mount_menu[mountable_volumes].v = &device_volumes[i];
                ++mountable_volumes;
            }
            if (is_safe_to_format(v->mount_point)) {
                sprintf(&format_menu[formatable_volumes].txt[0], "format %s", v->mount_point);
    ...
    for (;;)
    {
        ...
        chosen_item = get_menu_selection(headers, options, 0, 0);
        if (chosen_item == GO_BACK || chosen_item == REFRESH)
            break;
...
int show_nandroid_menu()
{
    ...
    for (;;) {
        ...
        LOGI("chosen_item=%d",chosen_item);
        if (chosen_item == GO_BACK || chosen_item == REFRESH)
            break;
