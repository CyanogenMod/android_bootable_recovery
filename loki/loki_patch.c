...
int loki_patch_main(int argc, char **argv) {
    ...
    char aboot_image[PATH_MAX];
    char in_image[PATH_MAX];
    char out_image[PATH_MAX];

	if (argc != 5 || strlen(argv[1]) > 8) {
		printf("Usage: %s [boot|recovery] [aboot.img] [in.img] [out.lok]\n", argv[0]);
		return 1;
	}

    if (strlen(argv[2]) > PATH_MAX || strlen(argv[3]) > PATH_MAX || strlen(argv[4]) > PATH_MAX) {
        printf("Image path too long!\n");
        return 1;
    }

    strcpy(partition_label, argv[1]);
    strcpy(aboot_image, argv[2]);
    strcpy(in_image, argv[3]);
    strcpy(out_image, argv[4]);

    return loki_patch(partition_label, aboot_image, in_image, out_image);
}
