...
int nandroid_backup(const char* backup_path, int backup_recovery, int backup_boot, int backup_system, int backup_data, int backup_cache, int backup_sdext, int backup_wimax)
{
    ...
    if (backup_wimax && !(ret = get_partition_device("wimax", tmp)))
    {
        ...
        ui_print("Generating wimax.%s md5 sum...\n", serialno);
        sprintf(tmp, "nandroid-md5.sh %s wimax.%s", backup_path, serialno);
        if (__system(tmp))
            ui_print("Error while generating wimax.%s md5 sum!\n", serialno);
        if (0 != ret)
    ...
    if (backup_sdext && (vol == NULL || 0 != stat(vol->device, &st)))
        ui_print("No sd-ext found. Skipping backup of sd-ext.\n");
    else{
        if (backup_sdext && ensure_path_mounted("/sd-ext"))
            ui_print("Could not mount sd-ext. sd-ext backup may not be supported on this device. Skipping backup of sd-ext.\n");
        else if (backup_sdext && (ret = nandroid_backup_partition(backup_path, "SDEXT:")))
            return ret;
    }
...
int nandroid_restore(const char* backup_path, int restore_boot, int restore_system, int restore_data, int restore_cache, int restore_sdext, int restore_wimax)
{
    ...
    sprintf(tmp, "cd %s && cat `ls *.md5 && ls -A .*.md5` > md5", backup_path); // dirrrrrty hack. Damn you, stupid busybox version of md5sum!
    __system(tmp);
