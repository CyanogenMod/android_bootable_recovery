...
{
    char android_secure_path[PATH_MAX];
    nandroid_backup_bitfield = 0;
    ui_set_background(BACKGROUND_ICON_INSTALLING);
    refresh_default_backup_handler();
    sprintf(android_secure_path, "%s/.android_secure", backup_path);
    ...
    if (is_data_media() || 0 != stat(android_secure_path, &s)) {
        ui_print("No .android_secure found. Skipping backup of applications on external storage.\n");
    }
    else {
        if (0 != (ret = nandroid_backup_partition_extended(backup_path, android_secure_path, 0)))
            return ret;
    }
...
int nandroid_restore(const char* backup_path, int restore_boot, int restore_system, int restore_data, int restore_cache, int restore_sdext, int restore_wimax)
{
    ...
    sprintf(android_secure_path, "%s/.android_secure", get_primary_storage_path());
    ...
    if (restore_data && 0 != (ret = nandroid_restore_partition_extended(backup_path, android_secure_path, 0)))
        return ret;
