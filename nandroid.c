...
static void nandroid_callback(const char* filename) {
    if (filename == NULL || nandroid_files_total == 0)
        return;
...
int nandroid_backup(const char* backup_path) {
    ...
    sprintf(tmp, "chmod -R 777 %s ; chmod -R u+r,u+w,g+r,g+w,o+r,o+w %s ; chmod u+x,g+x,o+x %s/backup", backup_path, base_dir, base_dir, base_dir);
    __system(tmp);
