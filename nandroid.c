...
static int tar_dump_wrapper(const char* backup_path, const char* backup_file_image, int callback) {
    char tmp[PATH_MAX];
    sprintf(tmp, "cd $(dirname %s); tar cv --exclude=data/data/com.google.android.music/files/* %s $(basename %s) 2> /dev/null", backup_path, strcmp(backup_path, "/data") == 0 && is_data_media() ? "--exclude 'media'" : "", backup_path);
...
int nandroid_dump(const char* partition) {
    ...
    // the 'stty raw' command is necessary if we don't want to do cr/lf conversions over adb,
    // which garbles our output. garbled output is bad, and even though you could issue the
    // command through the shell, it's safer to do it here
    __system("stty raw");
    ...
    if (strcmp(partition, "boot") == 0) {
        if (0 != (ret = nandroid_backup_partition("-", "/boot")))
            return ret;
    }

    if (strcmp(partition, "recovery") == 0) {
        if (0 != (ret = nandroid_backup_partition("-", "/recovery")))
            return ret;
    }

    if (strcmp(partition, "data") == 0) {
        if (0 != (ret = nandroid_backup_partition("-", "/data")))
            return ret;
    }

    if (strcmp(partition, "system") == 0) {
        if (0 != (ret = nandroid_backup_partition("-", "/system")))
            return ret;
    }
...
int nandroid_restore_partition_extended(const char* backup_path, const char* mount_point, int umount_when_finished) {
    ...
    sprintf(tmp, "%s/%s.img", backup_path, name);
    struct stat file_info;
    if (strcmp(backup_path,"-") && 0 != (ret = statfs(tmp, &file_info))) {
