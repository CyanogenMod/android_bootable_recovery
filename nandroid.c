...
int nandroid_restore_partition_extended(const char* backup_path, const char* mount_point, int umount_when_finished) {
    ...
    char tmp[PATH_MAX];
    struct stat file_info;
    if (0 == strcmp(name,".android_secure")) {
        char *part;
        int a = 0;
        while ((part = parts[a]) != NULL) {
            sprintf(tmp, "%s/%s.%s.vfat.img", backup_path, name, part);
            if (0 == (ret = statfs(tmp, &file_info))) {
                backup_filesystem = "vfat";
                restore_handler = unyaffs_wrapper;
                break;
            }
            sprintf(tmp, "%s/%s.%s.vfat.tar", backup_path, name, part);
            if (0 == (ret = statfs(tmp, &file_info))) {
                backup_filesystem = "vfat";
                restore_handler = tar_extract_wrapper;
                break;
            }
            sprintf(tmp, "%s/%s.%s.vfat.dup", backup_path, name, part);
            if (0 == (ret = statfs(tmp, &file_info))) {
                backup_filesystem = "vfat";
