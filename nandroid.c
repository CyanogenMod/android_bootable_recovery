...
static int tar_compress_wrapper(const char* backup_path, const char* backup_file_image, int callback) {
    char tmp[PATH_MAX];
    	snprintf(tmp, PATH_MAX, "twrpTar -c  %s -d %s -t %s.tar ", strcmp(backup_path, "/data") == 0 && is_data_media() ? "-m" : "", backup_path, backup_file_image);
...
static int tar_extract_wrapper(const char* backup_file_image, const char* backup_path, int callback) {
   ...
   return  tar_gzip_extract_wrapper(backup_file_image, backup_path, callback);
}
